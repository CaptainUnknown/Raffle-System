...
export default function Analytics() {
  ...
  const getTrending = async () => {
    ...
    if (timeFrame) {
      ...
      setTrendingNftData(responseSorted);
  ...
  const getMints = async () => {
    ...
    mints.forEach(async (mint) => {
      res.data.data.contracts.edges.forEach((item) => {
        if (mint.address.toLowerCase() === item.node.address.toLowerCase()) {
          ...
          allMintData.push({
            ...
            img: item.node.unsafeOpenseaImageUrl,
            slug: item.node.unsafeOpenseaSlug,
        ...
        {trendingNftData &&
          trendingNftData.map((item, idx) => (
            ...
            >
              <div className={styles.tableRow}>
                <div className={styles.collectionName}>
                  <div className={styles.avatarContainer}>
                    <img
                      className={styles.collectionAvatar}
                      src={item.node?.unsafeOpenseaImageUrl || placeHolder}
                      alt="collection logo"
                    />
                  </div>
                  <div className={styles.collectionTitle}>
                    <h4>
                      {item.node.name.length > 14
                        ? item.node.name.slice(0, 14) + "..."
                        : item.node.name}
                    </h4>
                    <p>
                      Circulating Supply:{" "}
                      {formatNum(item.node.circulatingSupply)}
                    </p>
                  </div>
                </div>
                <div className={styles.tableData}>
                  <img
                    ...
                    src="https://openseauserdata.com/files/6f8e2979d428180222796ff4a33ab929.svg"
                  />
                  {formatNum(item.node.stats.floor, 3)}
                </div>
                <div className={styles.tableData}>
                  {formatNum(item.node.stats.totalSales, 0)}
                </div>
                <div className={styles.tableData}>
                  <img
                    alt="eth logo"
                    src="https://openseauserdata.com/files/6f8e2979d428180222796ff4a33ab929.svg"
                  />
                  {formatNum(item.node.stats.average, 3)}
                </div>
                <div className={styles.tableData}>
                  <img
                    alt="eth logo"
                    src="https://openseauserdata.com/files/6f8e2979d428180222796ff4a33ab929.svg"
                  />
                  {formatNum(item.node.stats.volume, 2)}
                </div>
                <div className={styles.tableData}>
                  <img
                    alt="eth logo"
                  ...
                  />
                  {formatNum(
                    item.node.stats.floor * item.node.circulatingSupply,
                    0
                  )}
                </div>
              </div>
              ...
              >
                <a
                  href={`https://opensea.io/collection/${item.node.unsafeOpenseaSlug}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg
                ...
                <a
                  href={`https://looksrare.org/collections/${item.node.address}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg viewBox="0 0 148 148" xmlns="http://www.w3.org/2000/svg">
                ...
                <a
                  href={`https://etherscan.io/address/${item.node.address}`}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <svg
